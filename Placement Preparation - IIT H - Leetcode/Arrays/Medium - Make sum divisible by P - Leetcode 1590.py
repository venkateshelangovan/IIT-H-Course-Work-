...
Output: 2
Explanation: We cannot remove a single element to get a sum divisible by 9. The best way is to remove 
the subarray [5,2], leaving us with [6,3] with sum 9.
...
def MakeSumDivisibleByP(nums,p):
    total=sum(nums)
    rem=total%p 
    if rem==0:
        return 0 
    ...
    for i in range(len(nums)):
        ...
        r=sums%p
        search=(r-rem)%p
        if (search in rem_store):
            ans=min(ans,i-rem_store[search])
        rem_store[r]=i
