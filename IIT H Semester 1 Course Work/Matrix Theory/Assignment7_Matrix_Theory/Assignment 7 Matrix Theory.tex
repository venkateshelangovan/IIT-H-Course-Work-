...
%
\begin{lstlisting}
https://github.com/venkateshelangovan/IIT-Hyderabad-Assignments/tree/master/Assignment7_Matrix_Theory
\end{lstlisting}
%
\section{Problem}
...
Given,
\begin{align}
\vec{x}^T\myvec{1 & 0\\0 & -1}\vec{x}+2\myvec{-2 & -3}\vec{x}-6=0\label{given}
\intertext{where}
\vec{V}=\myvec{1 & 0 \\0 & -1}\\
\vec{u}=\myvec{-2 \\ -3}
\end{align}
...
\vec{c}&=\myvec{2\\-3}
\end{align}
The above equation \eqref{given} can be modified as 
\begin{align}
(\vec{x}+\vec{c})^T\vec{V}(\vec{x}+\vec{c})+2\vec{u}^T(\vec{x}+\vec{c})-6&=0\label{finalsub}
\end{align}
...
\end{align}
\begin{align}
    \vec{c}^T\vec{V}\vec{x}&=\myvec{2&-3}\myvec{1&0\\0&-1}\vec{x}=\myvec{2&3}\vec{x}\label{1.1}\\
    \vec{c}^T\vec{V}\vec{c}&=\myvec{2&-3}\myvec{1&0\\0&-1}\myvec{2\\-3}=-5\label{1.3}
\end{align}
Substituting the equations \eqref{1.1}, \eqref{1.3} in equation \eqref{eq1} we get 
\begin{align}
    &\implies\vec{x}^T\myvec{1&0\\0&-1}\vec{x}+2\myvec{2&3}\vec{x}-5\label{1f}
\end{align}
From equation \eqref{finalsub} consider,
\begin{align}
&\implies 2\vec{u}^T(\vec{x}+\vec{c})\\
&\implies 2\myvec{-2 & -3}\vec{x}+2\myvec{-2 & -3}\myvec{2\\ -3}\\
&\implies -2\myvec{2&3}\vec{x}+10\label{2f}
\end{align}

Substituting equations \eqref{1f}, \eqref{2f} in equation \eqref{finalsub} we get 
\begin{align}
    \vec{x}^T\myvec{1&0\\0&-1}\vec{x}+2\myvec{2&3}\vec{x}-2\myvec{2&3}\vec{x}+10-11&=0
\end{align}
\begin{align}
    \implies\vec{x}^T\myvec{1&0\\0&-1}\vec{x}-1&=0\label{result}
\end{align}
Given equation \eqref{given} is modified to equation \eqref{result} when the origin is moved to the point $\myvec{2\\-3}$ which is verified through the plot \ref{Fig :1}

From equations \eqref{result} ,\eqref{given} $\vec{V}$ doesn't change
\begin{align}
    \det(\vec{V})&=-1
\end{align}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\columnwidth]{Assignment 7.png}
    \caption{Hyperbola when origin is shifted}
    \label{Fig :1}
\end{figure}

The plot \ref{Fig :1} verifies the given hyperbola
\begin{align}
\vec{x}^T\myvec{1 & 0\\0 & -1}\vec{x}-\myvec{4 & 6}\vec{x}-6=0
\end{align}
...
\begin{align}
    ...
    \vec{v_{12}}&=\myvec{1\\-3}
\end{align} 
The given hyperbola equation \eqref{given} which is modified to the hyperbola
\begin{align}
